...
int main(int argc, char* argv[]) {
    ...
    if (result.count("g") == 1) {
        ...
        std::string dbname(result["n"].as<std::string>());
        std::string k(result["k"].as<std::string>());
        //std::cout << dbname << " " << k << endl;
        std::unique_ptr<groundupdb::IDatabase> db(GroundUpDB::loadDB(dbname));

        groundupdb::EncodedValue ev1 = db->getKeyValue(k);
        groundupdb::Bytes bytes = ev1.data();
        std::cout << "debug: " << bytes.size() << std::endl;
        for (int i = 0; i < bytes.size(); i++)
            std::cout << char(bytes[i]) << std::endl;
