...
{
  groundupdb::Bytes by = key.data();
  char* chars = new char[by.size() + 1];
  int pos = 0;
  for (auto& c :by) {
    chars[pos++] = (char)c;
  }
  chars[pos] = '\0';
  std::string mybytes(chars);
  //std::cout << "key : " << mybytes << std::endl;
  //std::string keyHash(std::to_string(key.hash()));
  std::ifstream t(mImpl->m_fullpath + "/" + mybytes + ".kv"); // DANGEROUS
  std::string mypath = mImpl->m_fullpath + "/" + mybytes + ".kv";
  ...
  std::cout << "bytes size1 : " << bytes.size() << std::endl;
